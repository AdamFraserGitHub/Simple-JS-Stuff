...
//urgh messy!
blockMemory[0] = [];//xCoord
blockMemory[1] = [];//yCoord
blockMemory[2] = [];//type
blockMemory[3] = [];//boundryBroken
...
function draw(){
	...
	for (var i = 1; i < counter; i++){
    	blockType[blockMemory[2][i]].x = blockMemory[0][i];
		blockType[blockMemory[2][i]].y = blockMemory[1][i];

		blockType[blockMemory[2][i]].drawBlock();
	}
	ctx.fill();
	ctx.closePath();
...
function staticBlockMemory(){
	for (var i = 1; i < counter; i++){
		blockMemory[0][i] = blockValues[0][i];
		blockMemory[1][i] = blockValues[1][i] * 300 - 15;
		blockMemory[2][i] = Math.floor(Math.random() * 5); //0~4 inclusive
	}
}
